<template>
	...
	<container-wrapper :designer="designer" :widget="widget" :parent-widget="parentWidget" :parent-list="parentList"
		...
		<div :key="widget.id" class="collapse-container data-table-container"
			...
			<!-- 查询条件面板 -->
			<el-card :key="widget.id" class="card-container" @click.native.stop="selectWidget(widget)" :size="widgetSize"
					...
					<div class="clear-fix" >
					 ...
					 <i v-if="widget.options.showFold" class="float-right" style="padding-top: 10px;"
						...
						<el-button-group class="float-right" :size="widgetSize">
							...
							<el-button type="" icon="el-icon-edit" >{{i18nt('designer.hint.reset')}}</el-button>
						</el-button-group>
					</div>
				</template>
		...
		<div v-show="menuVisible"
			...
			<div style="display: flex;justify-content: space-between;align-items: center;" >
				<p>自定义表格列</p>
				<el-button-group>
					<el-button type="primary" icon="el-icon-document" @click="saveCustomer">保存</el-button>
					<el-button type="primary" icon="el-icon-refresh" @click="resetCustomer">重置</el-button>
					<el-button type="primary" icon="el-icon-close" @click="menuVisible=false">关闭</el-button>
				</el-button-group>
			</div>
		  <el-table :data="widget.options.tableColumns" style="width: 100%"
		  	...
		  	height="480" border row-key="fieldName" ref="singleTable" stripe>
				...
				<el-table-column label="字段名" width="100" prop="fieldName">
					<template #default="scope">
						<el-input v-model="scope.row.fieldName" size="small" :disabled="true" ></el-input>
					</template>
				</el-table-column>
				...
				<el-table-column label="展示" width="70" prop="show">
					<template #default="scope">
						<el-switch v-model="scope.row.show" size="small"></el-switch>
					</template>
				</el-table-column>
				<el-table-column label="锁定" width="70" prop="fixed">
					<template #default="scope">
						<el-switch v-model="scope.row.fixed" size="small"></el-switch>
					</template>
				</el-table-column>
				<el-table-column label="排序" width="70" prop="sortable">
					<template #default="scope">
